...
SUPERVISOR_USER="${SUDO_USER:-$USER}"
...
case "$continue_prod" in
    "yes"|"y")
        ...
        NGINX_VHOSTS_YML="/usr/local/lib/python${PY_MAJ_MIN}/dist-packages/bench/playbooks/roles/nginx/tasks/vhosts.yml"
        if [ -f "$NGINX_VHOSTS_YML" ]; then
            sudo sed -i 's/when: nginx_vhosts/when: nginx_vhosts | length > 0/' "$NGINX_VHOSTS_YML" || true
        fi

        echo "🗒️ Temporarily commenting out nginx vhosts include in main.yml to avoid empty var crash..."
        NGINX_MAIN_YML="/usr/local/lib/python${PY_MAJ_MIN}/dist-packages/bench/playbooks/roles/nginx/tasks/main.yml"
        if [ -f "$NGINX_MAIN_YML" ]; then
            sudo sed -i 's/^\(\s*-\s*include_tasks:\s*vhosts\.yml\)/# \1/' "$NGINX_MAIN_YML" || true
            sudo sed -i 's/^\(\s*-\s*include:\s*vhosts\.yml\)/# \1/' "$NGINX_MAIN_YML" || true
        fi
